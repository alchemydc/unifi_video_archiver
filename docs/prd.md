...
| **Authentication** | Session-based (Cookie/CSRF) with auto-refresh on 401 errors. |
| **Validation** | `Zod` for runtime type-checking of incoming webhooks. |
| **Logging** | `Winston` with `DEBUG` toggle (log raw payloads, auth headers, and stream status). |
...
1. **Ingress Layer:** Express server listening for `POST` requests.
...
3. **Client Layer:** `UnifiClient` class handling the heavy lifting of NVR communication.
...
A standalone script (`src/bin/scout.ts`) to verify the NVR connection.
...
2. **Validation:** Verify the event contains a valid `cameraMac` and `timestamp`.
3. **Authentication Check:** Ensure the `UnifiClient` has a warm session.
4. **Buffer Delay:** Wait  seconds (configurable) to allow the NVR buffer to flush.
